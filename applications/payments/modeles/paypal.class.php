<?php
	...
	class paypal extends payments_abstract {
		public function generateForm($order_data = array()) {
			...
			$form .= '<input type="hidden" name="no_note" value="1">'."\n";
			$form .= '<input type="hidden" name="notify_url" value="http://' . $config['domain'] . '/payments/paypal/">'."\n";
			$form .= '<input type="hidden" name="return" value="http://' . $config['domain'] . '/payments/paypal/pdt/">'."\n";
			$form .= '<input type="hidden" name="cancel_return" value="http://' . $config['domain'] . '/product/' . $order_data['product_id'] . '/' . url($order_data['product_name']) . '">'."\n";
		...
		public function generateDepositForm($order_data = array()) {
			...
			$form .= '<input type="hidden" name="no_shipping" value="1">'."\n";
			$form .= '<input type="hidden" name="no_note" value="1">'."\n";
			$form .= '<input type="hidden" name="notify_url" value="http://' . $config['domain'] . '/payments/deposit_paypal/">'."\n";
			$form .= '<input type="hidden" name="return" value="http://' . $config['domain'] . '/payments/deposit_paypal/pdt/">'."\n";
			$form .= '<input type="hidden" name="cancel_return" value="http://' . $config['domain'] . '/account/deposit">'."\n";
			$form .= '<input type="hidden" name="image_url" value="">'."\n";
			$form .= '<input type="hidden" name="email" value="' . $this->getMemberData('email') . '">'."\n";
			$form .= '<input type="hidden" name="first_name" value="' . $this->getMemberData('firstname') . '">'."\n";
			$form .= '<input type="hidden" name="last_name" value="' . $this->getMemberData('lastname') . '">'."\n";
