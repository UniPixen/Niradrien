{if $member.super_elite_author  == 'true' || $member.elite_author  == 'true'}
	<section id="titre-page" class="{if $member.super_elite_author  == 'true'}auteur-super-elite{/if}{if $member.elite_author  == 'true'}auteur-elite{/if}">
		<div class="container">
			<div class="row">
	...
	<div id="tab-membre" class="dashboard container">
		{if check_login_bool() && $member.member_id == $smarty.session.member.member_id}
			<ul>
				...
				<li>
					<div></div>
					<a href="/member/collections">{$lang.collections}</a>
				</li>
				<li>
					<div></div>
					<a href="/account/deposit">{$lang.deposit}</a>
